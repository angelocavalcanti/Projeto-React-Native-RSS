...
const fetchItems = dispatch => async (feedURL) => { // rodando de forma assíncrona
    ...
    //console.log(" ====== FEED =====")
    //console.log(feed.rss.channel.item[0]);
    console.log('implementado');
    ...
    for (let i=0; i < qntFeeds; i++) {
        ...
        if (imageUrl !== '' && imageUrl !== undefined) {
            imageUrl = imageUrl.replace(/.*?(https:\/\/)/, 'https://'); // para extrair a url da imagem do texto
        }
       
        descricao = item.description.replace(/<[^>]+>/g, ''); // para retirar caracteres indesejados da descrição da notícia
        ...
        }] // atualiza o estado com cada novo feed (cada nova notícia)
    }
    // console.log("=================================================");
    // console.log("=================================================");
...
export const { Context, Provider } = createDataContext(
    ...
    // rssItems
    [] // inicia vazio para manter a consistência
);
